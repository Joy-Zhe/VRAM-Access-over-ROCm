...
# 默认目标
all: $(MATRIX_TARGET) $(BAR0_TARGET) $(DMA_TARGET)
...
clean:
	@echo "清理构建文件..."
	rm -f $(MATRIX_TARGET) $(BAR0_TARGET) $(DMA_TARGET)
	@echo "清理完成"
...
# 声明伪目标
.PHONY: all run-matrix run-bar0 run-dma clean check perf-matrix perf-bar0 perf-dma help
